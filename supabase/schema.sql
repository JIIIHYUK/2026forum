...
using (id = 'mars-terraforming-commons');

create policy "Anyone can create shared app state"
on public.app_state
for insert
to anon, authenticated
with check (id = 'mars-terraforming-commons');
...
create or replace function public.apply_scheduled_state_updates(
  ...
  p_support_interval_minutes integer default 30,
  p_promotion_delay_minutes integer default 15,
  p_promotion_pool_funding numeric default 30000000000,
  p_rank_grants numeric[] default array[1200000000, 950000000, 760000000, 620000000, 500000000, 420000000]
)
returns jsonb
language plpgsql
set search_path = ''
