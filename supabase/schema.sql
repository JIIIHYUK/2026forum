...
create or replace function public.app_state_reset_at(p_state jsonb)
returns timestamptz
language plpgsql
immutable
set search_path = ''
as $$
begin
  if p_state ? 'resetAt' and jsonb_typeof(p_state->'resetAt') = 'string' and length(p_state->>'resetAt') > 0 then
    return (p_state->>'resetAt')::timestamptz;
  end if;
  return null;
exception
  when others then
...
create or replace function public.app_state_authority_client_id(p_state jsonb)
returns text
language plpgsql
immutable
set search_path = ''
as $$
begin
  if p_state ? 'authorityClientId' and jsonb_typeof(p_state->'authorityClientId') = 'string' and length(p_state->>'authorityClientId') > 0 then
    return p_state->>'authorityClientId';
  end if;
  return null;
end;
$$;

create or replace function public.prevent_stale_app_state_write()
returns trigger
language plpgsql
set search_path = ''
as $$
declare
  incoming_reset_at timestamptz;
  current_reset_at timestamptz;
  incoming_authority_client_id text;
  current_authority_client_id text;
begin
  if new.id <> 'mars-terraforming-commons' and new.id not like 'mars-terraforming-commons:%' then
    return new;
  end if;

  incoming_reset_at := public.app_state_reset_at(new.state);
  incoming_authority_client_id := public.app_state_authority_client_id(new.state);

  select max(public.app_state_reset_at(state))
  into current_reset_at
  from public.app_state
  where id = 'mars-terraforming-commons' or id like 'mars-terraforming-commons:%';

  if current_reset_at is not null and (incoming_reset_at is null or incoming_reset_at < current_reset_at) then
    raise exception 'stale app_state write blocked';
  end if;

  select public.app_state_authority_client_id(state)
  into current_authority_client_id
  from public.app_state
  where id = 'mars-terraforming-commons:core'
     or id = 'mars-terraforming-commons'
  order by case when id = 'mars-terraforming-commons:core' then 0 else 1 end
  limit 1;

  if current_authority_client_id is not null and incoming_authority_client_id is distinct from current_authority_client_id then
    raise exception 'non-authority app_state write blocked';
  end if;
...
drop trigger if exists prevent_stale_app_state_write on public.app_state;
create trigger prevent_stale_app_state_write
before insert or update on public.app_state
for each row
execute function public.prevent_stale_app_state_write();

drop function if exists public.apply_scheduled_state_updates(
  text,
  boolean,
  boolean,
  integer,
...
create or replace function public.apply_scheduled_state_updates(
  p_state_id text,
  p_authority_client_id text default null,
...
declare
  ...
  expected_authority_client_id text;
begin
  select count(*) = 2
  ...
  expected_authority_client_id := public.app_state_authority_client_id(current_state);
  if expected_authority_client_id is not null and p_authority_client_id is distinct from expected_authority_client_id then
    raise exception 'non-authority scheduled update blocked';
  end if;
  ...
  if uses_segmented_state then
    ...
    set state = jsonb_build_object(
          'resetAt',
          current_state->'resetAt',
          'authorityClientId',
          current_state->'authorityClientId',
          'companies',
          current_companies
        ),
        updated_at = clock_timestamp()
