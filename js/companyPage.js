import { BUDGET_RULES, COLLABORATION_RULES, COMPANY_SPECIALTIES, TERRAFORMING_TARGETS } from "./config.js?v=12";
import {
...
import { requireCompanyAccess } from "./companyAuth.js?v=5";
...
const pendingResourceRows = (project, state) => {
  ...
  return resources
    .map((resource) => {
      ...
      const canBuyOnPlanet = ["iron", "titanium", "glass"].includes(resource.id);
          ...
          <div class="resource-actions">
            ${
              remaining > 0 && canBuyOnPlanet
                ? `<button class="ghost-button" type="button" data-buy-resource="${resource.id}" data-source="planet" data-pending-id="${project.id}">이 행성 구매 ${money(planetCost)}</button>`
                : ""
            }
            ${
              remaining > 0
        ...
        <article class="history-item pending-project">
          <div>
            ...
            <div class="resource-list compact-resource-list">
              ...
              <div class="resource-total">예상 자원 비용 ${money(project.evaluation.resourceTotalCost || 0)} · 프로젝트 비용 ${money(project.price)} · 20초마다 1% 손실 판정</div>
            </div>
          </div>
          <dl>
            ...
            <div><dt>손실 기록</dt><dd>${(project.resourceLosses || []).length}건</dd></div>
            ...
            <div class="pending-action"><dt>등록</dt><dd><button class="primary-button" type="button" data-complete-pending="${project.id}" ${project.resourceFulfilled ? "" : "disabled"}>자원 충족 후 등록</button></dd></div>
          </dl>
        </article>
      `
    )
