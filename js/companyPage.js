import { BUDGET_RULES, COLLABORATION_RULES, COMPANY_SPECIALTIES, TERRAFORMING_TARGETS } from "./config.js?v=16";
import {
  acceptCollaborationOffer,
  applyChairBenefit,
  applyChairSanction,
  applyDueElection,
  buyPendingProjectResource,
  completePendingProject,
...
import {
  ...
  signedPercent
} from "./aiEvaluator.js?v=17";
import { requireCompanyAccess } from "./companyAuth.js?v=7";
...
const resourceNow = (state) =>
  state.timeControl?.paused && state.timeControl?.pausedAt ? Number(state.timeControl.pausedAt) : Date.now();

const resourceProgress = (project, resourceId) => {
  const fulfilled = Number(project.fulfilledResources?.[resourceId] || 0);
  const inTransit = (project.resourceShipments || []).reduce(
    (sum, shipment) => sum + (shipment.status === "in_transit" && shipment.resourceId === resourceId ? Number(shipment.units || 0) : 0),
    0
  );
  return { fulfilled, inTransit };
};

const shipmentLine = (shipment, state) => {
  if (shipment.status === "arrived") return `${shipment.sourceLabel} 도착 완료`;
  if (shipment.status === "lost") return `${shipment.sourceLabel} 손실`;
  return `${shipment.sourceLabel} ${shipment.units}단위 · 도착 ${formatCountdown(Math.max(0, Number(shipment.arriveAt || 0) - resourceNow(state)))}`;
};
...
const powerMapMarkup = (parliament) => {
  ...
  const chairCompanyId = parliament.chairCompanyId ? String(parliament.chairCompanyId) : null;
  const viceChairCompanyId = parliament.viceChairCompanyId ? String(parliament.viceChairCompanyId) : null;
  const seats = [];
  ranking.forEach((row) => {
          ...
          .map(
            (row) => `
              <div class="power-legend-row">
                ...
                ${
                  String(row.companyId) === chairCompanyId
                    ? `<em class="power-badge chair-badge">의원장</em>`
                    : String(row.companyId) === viceChairCompanyId
...
const resourceRows = (evaluation) => {
  const resources = evaluation.resourceRequirements || [];
  if (!resources.length) return `<div class="resource-row"><span>필요 자원</span><strong>없음</strong><em>0억 원</em></div>`;
  return resources
    .map(
      (resource) => `
        <div class="resource-row">
          <span>${resource.label}</span>
          <strong>${resource.units}단위</strong>
          <em>${money(resource.cost)}</em>
        </div>
      `
    )
    .join("");
...
const pendingResourceRows = (project, state) => {
  const resources = project.requiredResources || project.resourceRequirements || project.evaluation.resourceRequirements || [];
  if (!resources.length) return `<div class="resource-row"><span>필요 자원</span><strong>없음</strong><em>충족</em></div>`;
  return resources
    .map((resource) => {
      const { fulfilled, inTransit } = resourceProgress(project, resource.id);
      const required = Number(resource.units || 0);
      const remaining = Math.max(0, required - fulfilled - inTransit);
      const shipments = (project.resourceShipments || []).filter((shipment) => shipment.resourceId === resource.id).slice(0, 3);
      const canBuyOnPlanet = true;
      const canMineAsteroid = ["iron", "titanium", "glass"].includes(resource.id);
      const planetCost = Number(resource.unitCost || 0) * remaining;
      const asteroidCost = Number(resource.unitCost || 0) * 2 * remaining;
      return `
        <div class="pending-resource-row">
          <div>
            <strong>${resource.label}</strong>
            <span>필요 ${required} · 도착 ${fulfilled} · 운송 ${inTransit} · 남음 ${remaining}</span>
            ${
              shipments.length
                ? `<small>${shipments.map((shipment) => shipmentLine(shipment, state)).join("<br>")}</small>`
                : `<small>주문 대기</small>`
            }
          </div>
          <div class="resource-actions">
            ${
              remaining > 0 && canBuyOnPlanet
                ? `<button class="ghost-button" type="button" data-buy-resource="${resource.id}" data-source="planet" data-pending-id="${project.id}">이 행성 조달 ${money(planetCost)}</button>`
                : ""
            }
            ${
              remaining > 0 && canMineAsteroid
                ? `<button class="primary-button" type="button" data-buy-resource="${resource.id}" data-source="asteroid" data-pending-id="${project.id}">주변 소행성에서 채굴 ${money(asteroidCost)}</button>`
                : remaining > 0
                  ? ""
                  : `<span class="resource-ready">충족</span>`
            }
          </div>
        </div>
      `;
...
const evaluationMarkup = (evaluation, label = "AI 평가") => `
  ...
  <div class="resource-list">
    ...
    ${resourceRows(evaluation)}
    <div class="resource-total">자원 비용 ${money(evaluation.resourceTotalCost || 0)} · 총 필요 비용 ${money(evaluation.totalRequiredCost || evaluation.effectivePrice || 0)}</div>
  </div>
...
const renderCompany = () => {
  ...
  companyProjectCount.textContent = `${company.projects.length}개 / 예비 ${(company.pendingProjects || []).length}개`;
  ...
  companyFunding.textContent = money(
    Number(company.governmentReceived || 0) + Number(company.rankGrantReceived || 0) + Number(company.adminGrantReceived || 0)
  );
...
const renderHistory = () => {
  ...
  if (!pendingProjects.length && !company.projects.length) {
  ...
  const pendingMarkup = pendingProjects
    .map(
      (project) => `
        <article class="history-item pending-project">
          <div>
            ...
            <div class="resource-list compact-resource-list">
              ${pendingResourceRows(project, state)}
              <div class="resource-total">예상 자원 비용 ${money(project.evaluation.resourceTotalCost || 0)} · 프로젝트 비용 ${money(project.price)} · 자원은 손실 없이 안전하게 도착</div>
            </div>
          </div>
          <dl>
            <div><dt>상태</dt><dd>${project.resourceFulfilled ? "자원 충족" : "자원 대기"}</dd></div>
            <div><dt>프로젝트 비용</dt><dd>${money(project.price)}</dd></div>
            <div><dt>예상 자원 비용</dt><dd>${money(project.evaluation.resourceTotalCost || 0)}</dd></div>
            <div><dt>손실 기록</dt><dd>손실 없음</dd></div>
            ...
            <div class="pending-action"><dt>등록</dt><dd><button class="primary-button" type="button" data-complete-pending="${project.id}" ${project.resourceFulfilled ? "" : "disabled"}>${project.resourceFulfilled ? "등록하기" : "자원 도착 후 등록"}</button></dd></div>
          </dl>
        </article>
      `
    )
    .join("");
  const registeredMarkup = company.projects
    .map(
      (project) => `
    ...
    .join("");
  projectHistory.innerHTML = `${pendingMarkup}${registeredMarkup}`;
  projectHistory.querySelectorAll("[data-complete-pending]").forEach((button) => {
    button.addEventListener("click", () => {
      const pending = getCompany(companyId).pendingProjects.find((project) => project.id === button.dataset.completePending);
      if (!pending) {
        saveStatus.textContent = "예비 프로젝트 없음";
        renderAll();
        return;
      }
      const result = completePendingProject(companyId, pending.id, resolveProjectOutcome(pending.evaluation));
      saveStatus.textContent = result.completed ? "자원 충족 등록 완료" : result.reason;
      if (result.completed) lastSavedEvaluation = result.project.evaluation;
      renderAll();
    });
  });
  projectHistory.querySelectorAll("[data-buy-resource]").forEach((button) => {
    button.addEventListener("click", () => {
      const result = buyPendingProjectResource(
        companyId,
        button.dataset.pendingId,
        button.dataset.buyResource,
        button.dataset.source
      );
      saveStatus.textContent = result.purchased ? `${result.units}단위 구매 완료` : result.reason;
      renderAll();
    });
  });
};
...
const renderPoliticsPanel = () => {
  ...
  const chairCompanyId = parliament?.chairCompanyId ? String(parliament.chairCompanyId) : null;
  const viceChairCompanyId = parliament?.viceChairCompanyId ? String(parliament.viceChairCompanyId) : null;
  const chair = chairCompanyId ? state.companies.find((item) => item.id === chairCompanyId) : null;
  const viceChair = parliament?.viceChairCompanyId
    ? state.companies.find((item) => item.id === viceChairCompanyId)
    : null;
  const isChair = chairCompanyId === company.id;
  const chairPowerUsed = Number(parliament?.chairPowerUsed || 0);
  const chairPowerLeft = Math.max(0, 3 - chairPowerUsed);
  const sanctionedCompanyIds = (parliament?.sanctionedCompanyIds || []).map(String);
  const sanctionTargets = state.companies.filter(
    (item) =>
      item.id !== company.id &&
      item.id !== viceChairCompanyId &&
      !sanctionedCompanyIds.includes(item.id) &&
      (item.projects || []).some((project) => !project.sanction)
  );
  const targetOptions = sanctionTargets
    .map((item) => `<option value="${item.id}">${item.name}${item.englishName && item.englishName !== item.name ? ` / ${item.englishName}` : ""}</option>`)
    .join("");
  const canSanction = chairPowerLeft > 0 && sanctionTargets.length > 0;
  ...
  politicsPanel.innerHTML = `
    ...
    <div class="politics-summary">
      <div><span>다음 선거</span><strong>${state.timeControl?.paused ? "정지 " : ""}${formatCountdown(getNextElectionMs(state))}</strong></div>
        ...
        ? `
          <div class="government-actions">
            <label class="compact-label">
              <span>제재 대상</span>
              <select id="sanctionTarget" ${canSanction ? "" : "disabled"}>${
                targetOptions || `<option value="">제재 가능한 프로젝트가 없습니다</option>`
              }</select>
            </label>
            <button class="ghost-button" id="sanctionButton" type="button" ${canSanction ? "" : "disabled"}>프로젝트 제재</button>
...
const saveProject = (projectInput, evaluation, extraBudget = 0) => {
  const result = createPendingProject(companyId, projectInput, evaluation, { extraBudget, plannedEvaluation: evaluation });
  lastSavedEvaluation = evaluation;
  saveStatus.textContent = result.created ? "예비 등록 완료" : result.reason;
  pendingProject = null;
  decisionPanel.classList.add("is-hidden");
  if (result.created) projectForm.reset();
  renderAll();
};
