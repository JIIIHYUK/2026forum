...
import { requestCompanyAccess } from "./companyAuth.js";
      ...
      (project) => `
        <article class="feed-item ${outcomeClass(project.evaluation)}">
          ...
          <div>
            ...
            <p class="feed-company-name">${projectCompanyNameBlock(project)} · ${project.field === "bio" ? "생화" : "물리"}</p>
          </div>
          <dl>
...
const render = () => {
  ...
  countdown.textContent = `정부 ${formatTime(getNextGrantMs(state))} / 선거 ${formatTime(getNextElectionMs(state))}`;
};
