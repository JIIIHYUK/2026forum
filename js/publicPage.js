import { BUDGET_RULES } from "./config.js?v=12";
import {
  applyDueElection,
  applyDueRankGrants,
...
import { requestCompanyAccess } from "./companyAuth.js?v=5";
import { mountMarsScene } from "./marsScene.js?v=2";
...
const projectCompanyNameBlock = (project) => `
  <span class="company-name-ko">${project.companyName}</span>
  ${
    project.companyEnglishName && project.companyEnglishName !== project.companyName
      ? `<span class="company-name-en">${project.companyEnglishName}</span>`
      : ""
  }
`;

const companyCard = (company) => `
  <a class="company-card" href="./company.html?id=${company.id}" data-company-id="${company.id}" aria-label="${company.name} 세부사항">
    <span class="company-card-mark">${company.mark}</span>
    <span>
      <strong class="company-name-stack">${companyNameBlock(company)}</strong>
    ...
    .map((company, index) => {
      ...
      return `
        <li>
          ...
          <span class="rank-name company-name-stack">${companyNameBlock(company)}</span>
      ...
      (project) => `
        <article class="feed-item ${outcomeClass(project.evaluation)}">
          ...
          <div>
            ...
            <p class="feed-company-name">${projectCompanyNameBlock(project)}</p>
          </div>
          <dl>
...
const render = () => {
  applyDueRankGrants();
  const state = applyDueElection();
  renderCompanies(state.companies);
  renderRanking(state);
  renderFeed(state);
  globalTerraforming.textContent = percent(getGlobalTerraforming(state));
  countdown.textContent = state.timeControl?.paused
    ? `정지 · 정부 ${formatTime(getNextGrantMs(state))} / 선거 ${formatTime(getNextElectionMs(state))}`
    : `정부 ${formatTime(getNextGrantMs(state))} / 선거 ${formatTime(getNextElectionMs(state))}`;
};
...
document.addEventListener("click", (event) => {
  const link = event.target.closest(".company-card[data-company-id]");
  if (!link) return;
  event.preventDefault();
  const companyId = link.dataset.companyId;
  if (requestCompanyAccess(companyId)) {
...
initializeSharedState().then(render);
