...
const FIELD_TARGETS = {
  general: new Set(TERRAFORMING_TARGETS.map((target) => target.id)),
...
const resourceMatches = {
  iron: ["구조", "기지", "프레임", "로봇", "시추", "운송", "파이프", "압력"],
  titanium: ["항공우주", "로켓", "추진체", "차폐", "방사선", "궤도", "페이로드", "착륙"],
  glass: ["온실", "태양광", "창", "돔", "광학", "관측", "패널"],
  electronics: ["ai", "자동", "제어", "모니터링", "센서", "데이터베이스", "로봇", "프로그래밍", "전자"],
  wood: ["생태", "거주", "서식", "농업", "배양", "생명"],
  stone: ["토양", "기초", "건설", "차폐", "석재", "지표", "굴착"],
  plastic: ["생명유지", "순환", "필터", "배관", "재활용", "막", "반응기", "플라스틱"]
};

const resourceRequirementFor = ({ source, costScale, methodReadiness, matchedKeywords }) => {
  const scale = clamp(Math.ceil(costScale + methodReadiness / 32), 1, 8);
  const requirements = RESOURCE_RULES.map((resource) => {
    const matches = (resourceMatches[resource.id] || []).filter((keyword) => source.includes(keyword.toLowerCase()));
    const units = matches.length ? clamp(matches.length + scale + Math.floor(matchedKeywords / 3), 1, 18) : 0;
    return {
      ...resource,
      units,
      cost: units * resource.unitCost
    };
  });
  if (!requirements.some((resource) => resource.units > 0)) {
    requirements.find((resource) => resource.id === "iron").units = scale;
    requirements.find((resource) => resource.id === "electronics").units = Math.max(1, Math.ceil(scale / 2));
    requirements.forEach((resource) => {
      resource.cost = resource.units * resource.unitCost;
    });
  }
  return requirements.filter((resource) => resource.units > 0);
};
...
export function evaluateProject(input, company) {
  ...
  const fieldBonus = field === "bio" ? 3.5 : field === "physics" ? 4.2 : 0;
  ...
  const profile = FIELD_IMPACTS[field] || FIELD_IMPACTS.general;
  const fieldTargets = FIELD_TARGETS[field] || FIELD_TARGETS.general;
  ...
  const resourceRequirements = resourceRequirementFor({ source, costScale, methodReadiness, matchedKeywords });
  const resourceTotalCost = resourceRequirements.reduce((sum, resource) => sum + resource.cost, 0);
  ...
  const totalRequiredCost = price + resourceTotalCost;
  const budgetShortage = Math.max(0, totalRequiredCost - availableBudget);
