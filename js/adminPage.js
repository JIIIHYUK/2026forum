import { BUDGET_RULES } from "./config.js?v=16";
import "./companyAuth.js?v=7";
import {
  applyDueElection,
...
import { impactClass, money, outcomeClass, percent, signedPercent } from "./aiEvaluator.js?v=17";
...
const adminBudgetForm = document.querySelector("#adminBudgetForm");
const adminBudgetCompany = document.querySelector("#adminBudgetCompany");
const adminBudgetAmount = document.querySelector("#adminBudgetAmount");
...
const renderAdmin = () => {
  ...
  const totalFunding = state.companies.reduce(
    (sum, company) =>
      sum + Number(company.governmentReceived || 0) + Number(company.rankGrantReceived || 0) + Number(company.adminGrantReceived || 0),
    0
  );
  ...
  const chair = chairCompanyId ? state.companies.find((company) => company.id === chairCompanyId) : null;
  const viceChair = parliament?.viceChairCompanyId
    ? state.companies.find((company) => company.id === viceChairCompanyId)
    : null;
  ...
  adminBudgetCompany.innerHTML = state.companies
    .map((company) => `<option value="${company.id}">${company.name}${company.englishName ? ` / ${company.englishName}` : ""}</option>`)
    .join("");
...
confirmResetButton.addEventListener("click", async () => {
  ...
  adminActionMessage.textContent = "초기화 중입니다.";
  try {
    await resetSharedState();
    adminActionMessage.textContent = "모든 값이 초기 상태로 돌아갔고 Supabase에도 반영되었습니다.";
  } catch (error) {
    console.warn("Supabase reset failed", error);
    resetState();
    adminActionMessage.textContent = "로컬은 초기화됐지만 Supabase 반영 중 오류가 발생했습니다.";
  } finally {
...
adminBudgetForm.addEventListener("submit", (event) => {
  event.preventDefault();
  const amount = Number(adminBudgetAmount.value || 0) * 100000000;
  const result = grantBudgetAsAdmin(adminBudgetCompany.value, amount);
  adminActionMessage.textContent = result.reason;
  if (result.granted) adminBudgetAmount.value = "";
  renderAdmin();
});
