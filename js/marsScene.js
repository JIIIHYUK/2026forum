...
export function mountMarsScene(canvas) {
  ...
  const innerMoon = new THREE.Mesh(new THREE.SphereGeometry(0.08, 16, 12), moonMaterial);
  innerMoon.position.set(1.72, 0.08, 0);
  moonOrbit.add(innerMoon);
  const outerMoon = new THREE.Mesh(new THREE.SphereGeometry(0.055, 14, 10), moonMaterial);
  outerMoon.position.set(-2.12, -0.05, 0.3);
  moonOrbit.add(outerMoon);
