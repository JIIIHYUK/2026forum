import { BUDGET_RULES, COLLABORATION_RULES, COMPANIES, RESOURCE_DELIVERY_RULES, RESOURCE_RULES } from "./config.js?v=16";
import {
...
} from "./supabaseStore.js?v=10";

const STORAGE_KEY = "marsTerraformingCommons.v1";
const SCHEDULE_LOCK_KEY = `${STORAGE_KEY}.scheduleLock`;
...
const withCompanyRuntimeDefaults = (company) => ({
  ...
  adminGrantReceived: Number(company.adminGrantReceived || 0),
...
const mergeConfigCompanies = (state) => {
  ...
  const parliament = state.politics?.parliament
    ? {
        ...
        seatsByCompany: Object.fromEntries(
          Object.entries(state.politics.parliament.seatsByCompany || {}).map(([companyId, seats]) => [String(companyId), seats])
        ),
        chairCompanyId: state.politics.parliament.chairCompanyId ? String(state.politics.parliament.chairCompanyId) : null,
        viceChairCompanyId: state.politics.parliament.viceChairCompanyId ? String(state.politics.parliament.viceChairCompanyId) : null,
        sanctionedCompanyIds: (state.politics.parliament.sanctionedCompanyIds || []).map(String),
        ranking: (state.politics.parliament.ranking || []).map((row) => ({
          ...row,
  ...
  return {
    ...
    politics: {
      ...
      parliament,
      electionHistory: (state.politics?.electionHistory || []).map((entry) => ({
        ...entry,
        chairCompanyId: entry.chairCompanyId ? String(entry.chairCompanyId) : null,
        viceChairCompanyId: entry.viceChairCompanyId ? String(entry.viceChairCompanyId) : null,
        seatsByCompany: Object.fromEntries(Object.entries(entry.seatsByCompany || {}).map(([companyId, seats]) => [String(companyId), seats])),
        ranking: (entry.ranking || []).map((row) => ({ ...row, companyId: String(row.companyId) }))
      })),
...
const shouldAcceptRemoteState = (remoteState) => {
  const rawLocal = window.localStorage.getItem(STORAGE_KEY);
  if (!rawLocal) return true;
  try {
    const localState = JSON.parse(rawLocal);
    const remoteResetMs = stateResetMs(remoteState);
    const localResetMs = stateResetMs(localState);
    if (remoteResetMs > localResetMs) return true;
    if (remoteResetMs < localResetMs) return false;
    return stateUpdatedMs(remoteState) >= stateUpdatedMs(localState);
  } catch {
    return true;
  }
};

const uniqueById = (...groups) => {
  const seen = new Set();
...
const projectContributionValue = (project) =>
  Number(project?.evaluation?.totalContributionPercent ?? project?.contribution ?? 0);

const projectContributionTotal = (projects = []) =>
  Number(projects.reduce((sum, project) => sum + projectContributionValue(project), 0).toFixed(3));

const mergeRemoteStateWithLocalProjects = (remoteState) => {
  const rawLocal = window.localStorage.getItem(STORAGE_KEY);
  if (!rawLocal) return remoteState;
  let localState;
  try {
  ...
  const remoteResetMs = Date.parse(remote.resetAt || "") || 0;
  const localResetMs = Date.parse(localState.resetAt || "") || 0;
  if (remoteResetMs > localResetMs) return remote;

  const localDeletedIds = localState.deletedProjectIds || [];
  const remoteDeletedIds = remote.deletedProjectIds || [];
  const deletedIds = new Set([...localDeletedIds, ...remoteDeletedIds]);

  remote.deletedProjectIds = Array.from(deletedIds);
  remote.projects = uniqueById(remote.projects || [], localState.projects || []).filter((project) => !deletedIds.has(project.id));
  remote.companies = (remote.companies || []).map((remoteCompany) => {
    const localCompany = (localState.companies || []).find((company) => company.id === remoteCompany.id) || {};
    const projects = uniqueById(remoteCompany.projects || [], localCompany.projects || []).filter((project) => !deletedIds.has(project.id));
    return {
      ...remoteCompany,
      projects,
      contribution: projectContributionTotal(projects),
      pendingProjects: uniqueById(remoteCompany.pendingProjects || [], localCompany.pendingProjects || []).filter(
...
const getScheduleLocks = () => {
  try {
    return JSON.parse(window.localStorage.getItem(SCHEDULE_LOCK_KEY) || "{}");
  } catch {
    return {};
  }
};

const acquireScheduleLock = (name, ttlMs = 5000) => {
  const now = Date.now();
  const locks = getScheduleLocks();
  if (locks[name] && now - Number(locks[name] || 0) < ttlMs) return false;
  locks[name] = now;
  window.localStorage.setItem(SCHEDULE_LOCK_KEY, JSON.stringify(locks));
  return true;
};

const releaseScheduleLock = (name) => {
  const locks = getScheduleLocks();
  delete locks[name];
  window.localStorage.setItem(SCHEDULE_LOCK_KEY, JSON.stringify(locks));
};
...
const queueRemoteWrite = (state) => {
  ...
  remoteWriteTimer = window.setTimeout(() => {
    pushRemoteState(state).catch((error) => {
      console.warn("Supabase queued state save failed", error);
    });
  }, 180);
};
...
const requestRemoteScheduledUpdates = (options = {}) => {
  ...
  remoteScheduleTimer = window.setTimeout(async () => {
    ...
    if (remoteState) {
      if (!shouldAcceptRemoteState(remoteState)) return;
      const mergedRemoteState = mergeRemoteStateWithLocalProjects(remoteState);
      applyingRemoteState = true;
      writeLocalState(mergeConfigCompanies(mergedRemoteState));
...
export async function initializeSharedState() {
  ...
  if (remoteState) {
    if (!shouldAcceptRemoteState(remoteState)) return getState();
    const mergedState = mergeConfigCompanies(mergeRemoteStateWithLocalProjects(remoteState));
    applyingRemoteState = true;
  ...
  await subscribeRemoteState((remote) => {
    if (!shouldAcceptRemoteState(remote)) return;
    const mergedRemoteState = mergeRemoteStateWithLocalProjects(remote);
    applyingRemoteState = true;
    writeLocalState(mergeConfigCompanies(mergedRemoteState));
    applyingRemoteState = false;
  });
...
export async function resetSharedState() {
  ...
  const pushed = await pushRemoteState(state);
  if (isSupabaseEnabled() && !pushed) {
    throw new Error("Supabase reset failed");
  }
  return state;
}
...
export function deleteProjectAsAdmin(companyId, projectId, type = "registered") {
  ...
  if (type === "pending") {
    ...
    company.pendingProjects.splice(pendingIndex, 1);
    state.deletedProjectIds = Array.from(new Set([...(state.deletedProjectIds || []), projectId])).slice(-500);
...
export function grantBudgetAsAdmin(companyId, amount) {
  const state = getState();
  const company = state.companies.find((item) => item.id === String(companyId));
  const grant = Math.max(0, Math.round(Number(amount || 0)));
  if (!company) return { state, granted: false, reason: "기업 정보를 찾을 수 없습니다." };
  if (!Number.isFinite(grant) || grant <= 0) return { state, granted: false, reason: "지급할 금액을 입력하세요." };

  company.budget = Number(company.budget || 0) + grant;
  company.adminGrantReceived = Number(company.adminGrantReceived || 0) + grant;
  company.projectAddsSinceFunding = 0;
  return { state: saveState(state), granted: true, reason: `${company.name}에 ${grant / 100000000}억 원을 지급했습니다.` };
}
...
export function applyDueRankGrants(force = false) {
  ...
  let now = timerNow(state);
  let elapsed = now - Number(state.lastRankGrantAt || now);
  if (!force && elapsed < intervalMs) return state;

  if (!acquireScheduleLock("rankGrant")) return getState();
  try {
    state = getState();
    if (state.timeControl?.paused && !force) return state;
    now = timerNow(state);
    elapsed = now - Number(state.lastRankGrantAt || now);
    if (!force && elapsed < intervalMs) return state;
...
export function skipRankGrantInterval() {
  ...
  writeLocalState({ ...state, updatedAt: nowIso() });
...
export function applyDueElection(force = false) {
  ...
  let now = timerNow(state);
  let elapsed = now - Number(state.politics?.lastElectionAt || now);
  if (!force && elapsed < intervalMs) return state;

  if (!acquireScheduleLock("election")) return getState();
  try {
    state = getState();
    if (state.timeControl?.paused && !force) return state;
    now = timerNow(state);
    elapsed = now - Number(state.politics?.lastElectionAt || now);
    if (!force && elapsed < intervalMs) return state;
    return runElection();
  } finally {
    releaseScheduleLock("election");
  }
}
